...
import { useGetRecordByIdQuery } from '../store/api/server.api';
import useCreateOrUpdateRecord from '../hooks/useCreateOrUpdateRecord';
import useDeleteRecord from '../hooks/useDeleteRecord';

import {
  ICountry, IState, ICity, IProfile, IRecord,
} from '../models/models';
...
export default function Record() {
  ...
  const {
    ...
    defaultValues: useMemo(() => (isEditing ? formData : false), [formData, isEditing]),
  });
  ...
  const onSubmit: SubmitHandler<IRecord> = async (formFinalData) => {
    if (isEditing) {
      const response = await updateRecord(numericId, formFinalData);
